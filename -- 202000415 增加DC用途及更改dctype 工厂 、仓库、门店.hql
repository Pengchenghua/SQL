...
CREATE
TEMPORARY TABLE IF NOT EXISTS csx_tmp.p_invt_1 AS -- 库存查询

SELECT dc_code,
       goods_code goodsid,
       sum(CASE WHEN sdt>=regexp_replace(${hiveconf:sdate},'-','') THEN qty END)inv_qty,
       sum(CASE WHEN sdt>=regexp_replace(${hiveconf:sdate},'-','') THEN amt END)inv_amt,
       sum(CASE WHEN sdt>=regexp_replace(date_sub(${hiveconf:edate},30),'-','') THEN qty END)inv_qty_30day,
       sum(CASE WHEN sdt>=regexp_replace(date_sub(${hiveconf:edate},30),'-','') THEN amt END)inv_amt_30day,
...
CREATE
TEMPORARY TABLE IF NOT EXISTS csx_tmp.p_sale_max AS
...
CREATE
TEMPORARY TABLE IF NOT EXISTS csx_tmp.p_entry_max AS
...
JOIN
  (SELECT receive_location_code,
   ...
   GROUP BY receive_location_code,
            goods_code) AS b ON a.receive_location_code=b.receive_location_code
...
CREATE
TEMPORARY TABLE IF NOT EXISTS csx_tmp.p_invt_2 AS
...
JOIN
  (SELECT location_code shop_id,
   ...
   WHERE sdt = 'current') b ON dc_code= b.shop_id
...
LEFT OUTER JOIN
  (SELECT shop_code AS shop_id,
   ...
   WHERE sdt = regexp_replace(${hiveconf:edate}, '-',''))d ON a.shop_id = d.shop_id
...
LEFT JOIN
  (SELECT goods_id,
   ...
   FROM csx_dw.goods_m
   WHERE sdt=regexp_replace(${hiveconf:edate}, '-',''))c ON a.goodsid = c.goods_id ;
