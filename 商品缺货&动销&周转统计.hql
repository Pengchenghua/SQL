...
from
	...
	(select 
	shop_code,
	product_codeset sdate= '2020-10-01';
set edate= '2021-02-28';
...
select 
    ...
    turnover_day,
    stock_out_rate
from csx_tmp.tmp_goods_02 a 
left join csx_tmp.tmp_goods_01 b on a.dc_code=b.shop_code  and a.division_code=b.root_category_code
left join csx_tmp.tmp_goods_03 c on a.dc_code=c.dc_code and a.mon=c.mon and a.division_code=c.division_code
left join csx_tmp.tmp_goods_04 d on a.dc_code=d.dc_code and a.mon=d.mon and a.division_code=d.division_code
left join (select shop_id,shop_name,sales_province_code,sales_province_name from csx_dw.dws_basic_w_a_csx_shop_m where sdt='current') e on a.dc_code=e.shop_id
left join (select distinct division_code,division_name from csx_dw.dws_basic_w_a_category_m where sdt='current') f on a.division_code=f.division_code
;
