  SELECT
	...
	c.product_name ,
	c.unit ,
	c.purchase_group_code ,
	c.purchase_group_name ,
    begin_qty ,
    begin_amt_no_tax ,
    begin_amt_tax ,
    end_qty ,
...
group by 
  ...
  ( select
        ...
        sum(case when move_type = '120A' then amt_no_tax*(1+tax_rate/100 )  end) `原料转成品额`,
        sum(case when move_type = '120A' then txn_qty end) `原料转成品量`,
    ...
    where
       ...
       -- and location_code ='W053'
        ...
        ) j on a.location_code =j.dc_code and a.product_code =j.goods_code
 left join 
 (select * from csx_dw.dws_basic_w_a_csx_product_info c where sdt='current') c
     on a.product_code=c.product_code and a.location_code=c.shop_code
;
  refresh csx_dw.dwd_wms_r_d_shipped_order_header;
 refresh csx_dw.dws_tms_r_d_entrucking_order_detail;
   
 select * from csx_dw.dws_wms_r_d_entry_detail  where sdt='20210114' and order_code='IN210114000388'; 
 select * from csx_dw.wms_entry_order where sdt='20210114';