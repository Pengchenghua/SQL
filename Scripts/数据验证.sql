SELECT
	...
	CASE WHEN SUM(sale_order)>SUM(sale_ods)
			THEN concat('sale_dw表数据缺失||',	ifnull(CAST(SUM(sale_order)-SUM(sale_ods) AS string),	''))
		WHEN SUM(sale_order)< SUM(sale_ods) 
			THEN concat('sale_dw表数据多于order_item||',ifnull(CAST(ABS(SUM(sale_order)-SUM(sale_ods))AS string),	''))
	ELSE '数据一致'
END note
FROM
(
...
GROUP BY
sdt
HAVING SUM(sale_order)-SUM(sale_ods)>1 or SUM(sale_order)-SUM(sale_ods)<-1
ORDER BY
sdt DESC;