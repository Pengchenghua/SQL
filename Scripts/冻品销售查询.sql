...
join 
(select pm.goods_id ,pm.goods_name,unit_name from csx_dw.dws_basic_w_a_csx_product_m  pm where sdt='current') h on a.goods_code =h.goods_id
...
select substr(sdt,1,6) mon,
	source_type,
	a.location_code ,
	shop_name,dist_code ,dist_name,prefecture_city ,prefecture_city_name,
	order_code ,
	goods_code ,
	b.goods_name,
	b.category_small_code ,
...
join 
(select goods_id ,goods_name,category_small_code ,category_small_name,classify_small_code ,classify_small_name 
from csx_dw.dws_basic_w_a_csx_product_m where sdt='current' and classify_middle_code = 'B0304') b on a.goods_code =b.goods_id
join 
(select sm.vendor_id ,vendor_name from csx_dw.dws_basic_w_a_csx_supplier_m sm where sdt='current')c on a.supplier_code =c.vendor_id
join 
(select location_code ,shop_name,dist_code ,dist_name,prefecture_city ,prefecture_city_name from csx_dw.csx_shop where sdt='current')d on a.location_code =d.location_code
where sdt>='20200801' and sdt<='20201031'
and receive_type_code like 'P%'
group by 
substr(sdt,1,6) ,
	source_type,
	a.location_code ,
...
join 
(select pm.goods_id ,pm.goods_name,unit_name from csx_dw.dws_basic_w_a_csx_product_m  pm where sdt='current') h on a.goods_code =h.goods_id
-- join csx_dw.date_m j on a.sdt=j.calday 
where sdt>='20200101'
	and sdt<'20201101'
and a.entry_type  like '采购%'
and receive_status=2
