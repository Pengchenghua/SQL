...
set yesterday     = regexp_replace(to_date(date_sub(current_timestamp(),1)),'-','');
set last_yesterday= regexp_replace(to_date(add_months(date_sub(current_timestamp(),1),-1)),'-','');
--本月
set mon     = regexp_replace(trunc(to_date(date_sub(current_timestamp(),1)),'MM'),'-','');
set last_mon= regexp_replace(trunc(to_date(add_months(date_sub(current_timestamp(),1),-1)),'MM'),'-','');
--本年
set year     = regexp_replace(trunc(to_date(date_sub(current_timestamp(),1)),'YY'),'-','');
set last_year= regexp_replace(trunc(to_date(add_months(date_sub(current_timestamp(),1),-1)),'YY'),'-','');

--本月
drop table if exists csx_tmp.temp_days_report02;

create table if not exists csx_tmp.temp_days_report02 as
select
	province_code  ,
	province_name  ,
	...
	case when dc_code='W0H4' then item_channel	else regexp_replace(channel_name,'\\s','') end channel_name,
	count(distinct case	when sdt >= ${hiveconf:mon}	and sdt <= ${hiveconf:yesterday} then goods_code end) as sale_sku,
	sum	(case when sdt   >= ${hiveconf:mon}	and sdt<=${hiveconf:yesterday} then sales_value 	end	) sale,
	sum	(case when sdt   >= ${hiveconf:last_mon} and sdt<=${hiveconf:last_yesterday} then sales_value end ) as last_sale,
	...
	count(distinct	case when sdt   >= ${hiveconf:mon}	and sdt<=${hiveconf:yesterday}	then customer_no end) as sale_cust,
	count(distinct case when sdt   >= ${hiveconf:last_mon}	and sdt<=${hiveconf:last_yesterday} then customer_no end) as last_sale_cust
from
	csx_dw.dws_sale_r_d_customer_sale a 
	left join 
	(select category_small_code,purchase_group_code as department_code,purchase_group_name as department_name from csx_dw.dws_basic_w_a_category_m where sdt='current')b 
...
group by
	...
	case when dc_code='W0H4'     then item_channel	else regexp_replace(channel_name,'\\s','') end
union all
select
	province_code          ,
	province_name          ,
	...
	count(distinct case	when sdt   >= ${hiveconf:mon} and sdt<= ${hiveconf:yesterday} then goods_code end) as sale_sku,
	sum	(case when sdt   >= ${hiveconf:mon}	and sdt<=${hiveconf:yesterday}	then sales_value end) sale,
	sum	(case when sdt   >= ${hiveconf:last_mon} and sdt<=${hiveconf:last_yesterday} then sales_value end)	last_sale,
	sum	(case when sdt   >= ${hiveconf:mon}	and sdt<=${hiveconf:yesterday} then profit	end	) profit,
	sum	(case when sdt   >= ${hiveconf:mon}	and sdt<=${hiveconf:yesterday}	then front_profit	end	) front_profit,
	count(distinct case	when sdt   >= ${hiveconf:mon} and sdt<=${hiveconf:yesterday} then customer_no end) as sale_cust,
	count(distinct case when sdt   >= ${hiveconf:last_mon}	and sdt<=${hiveconf:last_yesterday} then customer_no end) as last_sale_cust
...
SELECT
	...
	case when division_code in ('11','10') then '11' when division_code in ('12','13','14')   then '12' else division_code end as division_code     ,
	case when division_code in ('11','10') then '生鲜采购部' when division_code in ('12','13','14')   then '食百采购部' else division_name end as division_name   ,
	case when division_code='10' then 'U00' else b.department_code end as	department_code      ,
case when division_code='10' then '加工课' else b.department_name end  as 	department_name      ,
	'全渠道' AS channel_name,
	count(DISTINCT
	CASE
		WHEN sdt   >= ${hiveconf:mon}
...
CREATE table if not exists csx_tmp.temp_days_report03 as
...
SELECT
	...
	sum	(CASE	WHEN sdt   =${hiveconf:yesterday}	THEN sales_value END) sale,
	sum	(CASE	WHEN sdt =${hiveconf:last_yesterday} THEN sales_value	END)	last_sale,
	sum
		(
			CASE
				WHEN  sdt =${hiveconf:yesterday}
...
SELECT
    ...
    case when division_code='00' then  
            (sum(coalesce(sale,0))/sum(sum(coalesce(sale,0)))over(partition BY date_m,regexp_replace(channel_name,'\\s','')
                                                           ORDER BY date_m))*6.00
    else  (sum(coalesce(sale,0)) /sum(sum(coalesce(sale,0)) )over(partition by date_m,province_code,regexp_replace(channel_name,'\\s','') order by date_m ))*3.00 end  as sale_ratio,
...
SELECT
	...
	department_code,
	department_name,
	regexp_replace(channel_name,'\\s','')  channel_name ,
	sale_sku       ,
	sale           ,
	last_sale      ,
	profit         ,
