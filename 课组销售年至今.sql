...
if not exists csx_tmp.temp_days_report01 as
	SELECT
		province_code ,
		province_name ,
		case
	...
	INSERT overwrite table csx_dw.ads_sale_r_m_dept_sale_mon_report partition
		(sdt
		)
	SELECT
		date_m         ,
		...
		case
			when division_code='00'
				then (sum(coalesce(sale,0))/sum(sum(coalesce(sale,0)))over(partition BY date_m,channel_name ORDER BY
																		   date_m))*6.00
				else (sum(coalesce(sale,0)) /sum(sum(coalesce(sale,0)) )over(partition by date_m,province_code,channel_name order by
																			 date_m ))*3.00
		end                                                             as sale_ratio      ,
