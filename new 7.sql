...
CREATE TEMPORARY TABLE IF NOT EXISTS temp.scm_order_01 AS
SELECT
    a.order_code                                  ,
    super_class                                   ,
    original_order_code                           ,
    ...
    purchase_group_code as dept_id                  ,
...
SELECT
    regexp_replace(to_date(a.order_update_time),'-','')order_date , --订单日期
    a.order_code                                                  , --订单号
    ...
    LEFT OUTER JOIN
        (
            SELECT
                ...
                receive_time                                   , -- 收货时间
                close_time as receive_close_time               , -- 关单时间
                close_by                                       , -- 关单人
                auto_status                                    , --是否自动执行
                sale_channel                                   , -- 销售渠道
