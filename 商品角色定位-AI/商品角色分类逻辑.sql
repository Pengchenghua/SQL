...
--C：结构商品，sku数量占比30%，此类商品多用以完善品类结构和满足客户个性化需求，属低频补充性商品，应赚取相应毛利；
--注：各品类及各省区间或会因实际业务情况产生差异，上述sku数量占比数值可根据实际情况进行调整；
--【区分规则】
--根据商品销售额（权重30%）、销量（权重30%）、动销天数（权重20%）、客户渗透率（权重20%）进行商品分级；
--①仅针对基础商品池内商品进行商品分级；
--②以品类维度（二级品类），针对品类下商品进行数据分析；
--③根据数据按照月度销售额、销量、动销天数、客户渗透率排名进行综合排名（剔除贸易大宗业绩、季节性明显上商品）；
--④按照排名及sku占比，确认最终商品角色
...
(
  select a.*,
  --综合得分：商品销售额（权重30%）、销量（权重30%）、动销天数（权重20%）、客户渗透率（权重20%）
