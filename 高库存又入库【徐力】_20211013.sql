...
--滞销商品明细360未销售
-- 第一次未销售商品库存
drop table if exists csx_tmp.tmp_no_sale_goods_01 ;
create temporary table  csx_tmp.tmp_no_sale_goods_01 as 
SELECT a.dist_code,
       a.dist_name,
       a.dc_code,
       a.dc_name,
...
WHERE    
    sdt='20211013'             --更改查询日期
  AND a.no_sale_days>360
  ;
  


--期间入库数据
create temporary table csx_tmp.tmp_no_sale_goods_03 as 
select order_code,
    origin_order_code,
    receive_location_code as dc_code,
    goods_code,
    ...
    sum(price*receive_qty) receive_amt
from csx_dw.dws_wms_r_d_entry_detail
where sdt>regexp_replace(date_add(current_date(),-360),'-','')
    and sdt<'20211013'
    and order_type_code like 'P%'
    and receive_status='2'
group by
...
--关联最后一次&第一次 两次同时高库存&关联期间入库数据
create temporary table csx_tmp.tmp_no_sale_goods_04 as 
select a.dist_code,
       a.dist_name,
       a.dc_code,
       a.dc_name,
       ...
       c.receive_qty,
       order_array,
       order_cn from csx_tmp.tmp_no_sale_goods_01 a 
left join
(select 
    concat_ws(',',collect_set(origin_order_code)) as order_array,
    dc_code,
    ...
    sum(receive_qty) receive_qty,
    sum(receive_amt) receive_amt
from csx_tmp.tmp_no_sale_goods_03
group by  dc_code,
    goods_code
)c on a.dc_code=c.dc_code and a.goods_id=c.goods_code
;
select * from  csx_tmp.tmp_no_sale_goods_04 where final_qty>1 ;