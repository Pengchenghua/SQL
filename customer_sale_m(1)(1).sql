...
from (
    select
      ...
      case
        ...
        when b.channel = '大客户'
        or b.channel = 'B端' then '1'
        when b.channel = 'M端'
        or b.channel like '%对内%' then '2'
      ...
      case
        ...
        and b.sales_province = 'BBC' then '福建省'
        when b.sales_province is not null
        and b.channel <> 'M端'
        and b.channel not like '商超%' then b.sales_province
        when a.customer_no like 'S%'
        and substr(c.province_name, 1, 2) in (
          '重庆',
          '四川',
          '北京',
          '福建',
    ...
    from (
        ...
        where
          ...
          and sdt >= concat(
            substr(
              regexp_replace(
                add_months(date_sub(current_date, 1), -1),
                '-',
                ''
              ),
              1,
    ...
    left outer join (
        select
          customer_no,
          sign_time,
          split(sign_time, ' ') [0] as sign_date,
          regexp_replace(
            date_sub(
              split(sign_time, ' ') [0],
              pmod(
                datediff(split(sign_time, ' ') [0], '2012-01-07'),
                7
              )
            ),
            '-',
          ...
          regexp_replace(trunc(split(sign_time, ' ') [0], 'MM'), '-', '') as sign_month,
          sales_province_code,
          sales_province,
          sales_city,
          channel
...
select
  ...
  a.channel_name,
  case
    when a.province_name = '商超平台' then '-100'
    else g.province_code
  end province_code,
  case
    when a.province_name = '平台-B' then '大客户平台'
...
from (
    select
      ...
      case
        ...
        or channel = '' then '大客户'
        when province_name = '平台-B'
        and channel = '1' then '大客户'
        else channel_name
      end channel_name,
      case
        when province_name = '成都省' then '四川省'
        when channel = '7' then '福建省'
        else province_name
      end province_name,
      case
        when channel = '2' then city_name --when channel='7' then '福州'
        when (channel <> '2') then region_city
        else '-'
      end city_name
