...
select dc_code,
       ...
       sum(case when classify_middle_code not in ('B0401','B0902') then   cost_30day end) cost_30day,
       sum(case when classify_middle_code not in ('B0401','B0902') THEN  period_inv_amt_30day end) period_inv_amt_30day,
...
where sdt=${hiveconf:enddt} 
    ...
    ) b on a.goods_id=b.product_code AND a.dc_code=b.shop_code
where sdt=${hiveconf:enddt}
...
from  csx_tmp.temp_sku a
left join
-- 存储SKU
csx_tmp.temp_turnover   b on b.dc_code=a.shop_code and a.classify_small_code=b.classify_small_code 
left join 
-- 存储商品销售
csx_tmp.temp_sale_01 c on a.shop_code=c.dc_code and a.classify_small_code=c.classify_small_code
left join
-- 客户满足率
 csx_tmp.temp_cust_sale_01 d on a.shop_code=d.dc_code and a.classify_small_code=d.classify_small_code
 left join 
 csx_tmp.temp_supplier_fill_rate    j on a.shop_code=j.receive_location_code and a.classify_small_code=j.classify_small_code      --供应商满足率
left join csx_tmp.temp_pin k on a.shop_code=k.dc_code and a.classify_small_code=k.classify_small_code
;


-- select sum(no_stock_goods_amt) from  csx_tmp.temp_turnover where dc_code='W0A2' AND classify_middle_name='酒';
...
-- drop table  csx_tmp.temp_report_scm_r_d_purchase_goods_fr;
-- create temporary table csx_tmp.temp_report_scm_r_d_purchase_goods_fr as 
select 
    province_code,
    province_name,
    city_code,
    ...
    round(order_qty,6 )  order_qty,     --订单数量
    round(order_amt/10000 ,6) order_amt ,   --订单金额
    round(receive_qty,6)    receive_qty ,   --入库数量
    round(receive_amt/10000,6)  receive_amt ,   --入库金额
    round(receive_qty  /order_qty,6)  order_sign_rate,       --满足标识 
    round(zs_order_amt/10000,6) zs_order_amt,       --客户直送订单金额
    round(zs_receive_amt/10000,6)  zs_receive_amt,         --客户直送金额
    round(zs_receive_amt/receive_amt,6) zs_receive_ratio,   --客户直送入库占比
    round(order_value/10000 ,6 ) order_value,     --客户配送订单金额 剔除地采 
    round(sales_value/10000 ,6 ) sales_value,     --客户配送出库金额 剔除地采
    round(sales_value/order_value,6 ) cust_sale_ratio,       -- 客户配送满足率
    round(daily_sales_value/10000 ,6)  daily_sales_value,     --日配销售金额
    round(stock_sale_value/10000  ,6) stock_sale_value,       --存储商品销售额
    round(stock_sale_value/daily_sales_value,6) as stock_sale_ratio, -- 存储商品销售占比
    ...
    round(stock_goods_return_sku/stock_goods_sku,6) as stock_goods_return_sku_ratio,  -- 存储商品可退占比
    round(shipp_amt/10000 ,6 ) shipp_amt,                     --退货金额
    round(final_qty       ,6 )  end_inventory_qty ,                   --库存金额
    round(final_amt/10000 ,6 ) end_inventory_amt ,                   --库存金额
  --   (cost_30day )  cost_30day,                   --30天成本
 --    (period_inv_amt_30day )/10000 period_inv_amt_30day,        --30天期间库存金额
    round(period_inv_amt_30day /cost_30day,6 )  days_turnover_30,  --30天周转
    (in_stock_sku )  in_stock_sku,                --库存SKU   
    (pin_sku )  pin_sku ,                         --动销SKU
    (no_pin_sku)no_pin_sku,
    round( no_pin_sku/all_sku ,6) as no_pin_sku_rate,      -- 无动销率
    (stock_goods_inventory_sku) stock_goods_inventory_sku,       -- 存储商品有库存SKU
    round(stock_goods_inventory_amt/10000,6) stock_goods_inventory_amt,    -- 存储商品库存金额
    (stockout_goods_sku) stockout_goods_sku,
    round(stockout_goods_sku/stock_goods_sku,6) as stockout_goods_sku_rate,  -- 存储商品缺货率
    (no_stock_goods_sku) no_stock_goods_sku,       -- 非存储商品SKU
    round(no_stock_goods_amt/10000,6) no_stock_goods_amt,    --  非存储商品库存金额 
    current_timestamp() update_time,   
    substr(${hiveconf:enddt},1,6) sdt,
     'M'  dim_date    
...
set shop=('W0A2','W0A3','W0A7','W0A8','W0N0','W0A5','W0A6');
set enddt=regexp_replace('${enddate}','-','');
set sdt=regexp_replace(date_add(next_day('${enddate}','MO'),-7),'-','');

-- select  ${hiveconf:enddt},${hiveconf:sdt};

-- 1.0 供应商满足率
drop table  csx_tmp.temp_supplier_fill_rate_w;
create temporary table csx_tmp.temp_supplier_fill_rate_w as 
select 
a.receive_location_code ,
classify_large_code,
classify_large_name,
...
drop table  csx_tmp.temp_turnover_w;
create temporary table csx_tmp.temp_turnover_w as
select dc_code,
       a.classify_large_code,
       a.classify_large_name,
       a.classify_middle_code,
...
-- 3.0 储存属性为库存 销售占比
drop table  csx_tmp.temp_sale_01_w;
create temporary table  csx_tmp.temp_sale_01_w as 
select dc_code,
    c.classify_large_code,
    c.classify_large_name,
    c.classify_middle_code,
...
--4.0 可退商品SKU占比
drop table  csx_tmp.temp_sku_w;
create temporary table csx_tmp.temp_sku_w as 
select 
    shop_code,
    classify_large_code,
    classify_large_name,
...
-- 7.0 无动销SKU
drop table csx_tmp.temp_pin_w ;
create temporary table  csx_tmp.temp_pin_w as 
select a.dc_code,
    classify_large_code,
    classify_large_name,
    classify_middle_code,
...
-- 客户满足率  剔除客户直送\福利单出库
drop table csx_tmp.temp_cust_sale_01_w;
create temporary table csx_tmp.temp_cust_sale_01_w as  
select dc_code,
      classify_large_code,
    classify_large_name,
    classify_middle_code,
...
drop table csx_tmp.temp_all_02_w;
create temporary table csx_tmp.temp_all_02_w as 
select 
     a.shop_code dc_code,
    case when a.classify_large_code in ('B01','B02','B03') then '11' else '12' end  div_code,
    case when a.classify_large_code in ('B01','B02','B03') then '生鲜' else '食百' end div_name ,
   ...
   b.final_amt , 
    ...
    (period_inv_amt_30day)/(cost_30day) as days_turnover_30,
     ...
     no_stock_goods_sku,       -- 非存储商品SKU
     no_stock_goods_amt    --  非存储商品库存金额 
from  csx_tmp.temp_sku_w a
left join
-- 存储SKU
csx_tmp.temp_turnover_w   b on b.dc_code=a.shop_code and a.classify_small_code=b.classify_small_code 
left join 
-- 存储商品销售
csx_tmp.temp_sale_01_w c on a.shop_code=c.dc_code and a.classify_small_code=c.classify_small_code
left join
-- 客户满足率
 csx_tmp.temp_cust_sale_01_w d on a.shop_code=d.dc_code and a.classify_small_code=d.classify_small_code
 left join 
 csx_tmp.temp_supplier_fill_rate_w    j on a.shop_code=j.receive_location_code and a.classify_small_code=j.classify_small_code      --供应商满足率
left join csx_tmp.temp_pin_w k on a.shop_code=k.dc_code and a.classify_small_code=k.classify_small_code
;


-- select sum(no_stock_goods_amt) from  csx_tmp.temp_turnover where dc_code='W0A2' AND classify_middle_name='酒';

drop table    csx_tmp.temp_all_00_w ;
create temporary table csx_tmp.temp_all_00_w as    
  select 
    dc_code ,
    div_code,
    div_name,
...
select 
 ...
 --    (period_inv_amt_30day )/10000 period_inv_amt_30day,        --30天期间库存金额
    ...
    round(no_stock_goods_amt/10000,6) no_stock_goods_amt,    --  非存储商品库存金额 
    current_timestamp() update_time,   
     IF(length( concat(year(date_sub(next_day('${enddate}' ,'MO'),4)), '', weekofyear('${enddate}') ))=6, 
        concat(year(date_sub(next_day('${enddate}' ,'MO'),4)), '', weekofyear('${enddate}') ),
         concat(year(date_sub(next_day('${enddate}' ,'MO'),4)), '0', weekofyear('${enddate}') )) as  sdt,
     'W'  dim_date    
from csx_tmp.temp_all_00_w a 
left join
(select shop_id,shop_name,province_name,province_code,city_code,city_name from csx_dw.dws_basic_w_a_csx_shop_m where sdt='current') b on a.dc_code=b.shop_id
;
