...
`business_attribute_code` STRING  COMMENT '客户属性编码',
`business_attribute_name` STRING  COMMENT '客户属性',
`customer_level` STRING  COMMENT '客户信控等级',
`customer_level_name` STRING  COMMENT '客户信控等级名称',
`first_category_code` STRING  COMMENT '客户一级分类编码',
`first_category_name` STRING  COMMENT '客户一级分类名称',
`second_category_code` STRING  COMMENT '客户二级分类编码',
`second_category_name` STRING  COMMENT '客户二级分类名称',
`third_category_code` STRING  COMMENT '客户三级分类编码',
`third_category_name` STRING  COMMENT '客户三级分类名称',
...
`sales_employee_code` STRING  COMMENT '销售员工号',
`sales_employee_name` STRING  COMMENT '销售员名称',
`supervisor_user_id` STRING  COMMENT '一级主管编码 b端客户：销售主管,s端：采购总监 大宗：主管',
...
(
  ...
  third_category_name ,
  attribute_desc,
  customer_level_code  ,           -- 客户等级编码
  `customer_level_name`            -- 客户等级名称
