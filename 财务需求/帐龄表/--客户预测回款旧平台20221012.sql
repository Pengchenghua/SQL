  ...
  from csx_dw.dws_sap_r_d_customer_settle_detail
   ...
   (select channel_name,
    ...
    from csx_dw.dws_crm_w_a_customer 
        where sdt='current'
            and customer_no!='') b on  a.customer_code = b.customer_no
    ;
...
-- select * from csx_tmp.temp_ads_fr_r_d_forecast_collection_report_01 where customer_no like '125913';
 ...
 select 
   ...
   t.second_supervisor_work_no,
   t.second_supervisor_name,
   t.third_supervisor_work_no,
   t.third_supervisor_name,
   t.fourth_supervisor_work_no,
   t.fourth_supervisor_name,
   t.credit_limit ,
   t.temp_credit_limit ,
   t.payment_terms,
   t.payment_name,
