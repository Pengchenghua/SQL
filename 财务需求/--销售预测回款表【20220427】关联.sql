...
from
(select a.comp_code,
	   coalesce(a.customer_no,'')customer_no,
	   province_code,
	   sum(ac_all) ac_all,
...
select 
   ...
   a.payment_name,
   a.payment_days,
   a.payment_terms as zterm ,
   cast(a.payment_days as int)  diff ,
   back_money_amount_month
  from 
  (
  select *
  from csx_dw.dws_sap_r_d_customer_settle_detail
    where 1=1 
    ...
    where 
        sdt='current'  ) c on a.customer_code = c.customer_no and a.company_code = c.company_code
     left join 
   (select channel_name,
           customer_no,
           customer_name,
    ...
    from csx_dw.dws_crm_w_a_customer 
        where sdt='current'  ) b on  a.customer_code = b.customer_no
    ;
...
select 
   ...
   -- 当预测回款金额<0 则0-当前回款，则正常计算
  if(coalesce(receivable_amount_target,0)<=0,0-coalesce(back_money_amount_month*-1,0),coalesce(receivable_amount_target,0)-coalesce(back_money_amount_month*-1,0)) as need_receivable_amount , --需回款金额=预测回款目标-当期回款金额
   0 temp_1,
   0 temp_2,
   0 temp_3,
...
--insert overwrite table csx_tmp.ads_fr_r_d_forecast_collection_report partition (sdt)
...
from csx_tmp.temp_ads_fr_r_d_forecast_collection_report_02 t
 ...
 select sum(current_receivable_amount),sum(need_receivable_amount) from csx_tmp.temp_acc_01