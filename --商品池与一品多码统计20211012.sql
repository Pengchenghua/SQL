...
select a.province_code,
       ...
       count(distinct case when des_specific_product_status not in ('0','7') then product_code end  ) error_status_sku
...
select distinct des_specific_product_status,product_status_name from csx_dw.dws_basic_w_a_csx_product_info where sdt='current'
;
