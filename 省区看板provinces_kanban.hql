...
from (
    ...
    sum(CASE when sdt =  ${hiveconf:edate} then sales_value end ) day_sale,
		sum(CASE when sdt =  ${hiveconf:edate} then profit end) day_profit,
		...
		sum(CASE when sdt >=  ${hiveconf:l_sdate} and sdt <= ${hiveconf:l_edate} then sales_value end ) mom_sale,
		sum(CASE when sdt >=  ${hiveconf:l_sdate}       and sdt <= ${hiveconf:l_edate} then profit end)  mom_profit,
		sum(CASE when sdt >=  ${hiveconf:y_sdate}       and sdt <= ${hiveconf:y_edate}  then sales_value end ) yoy_sale,
		sum(CASE when sdt >=  ${hiveconf:y_sdate}       and sdt <= ${hiveconf:y_edate}  then profit end)  yoy_profit
		from
			csx_dw.customer_sale_m1 a 
		where
...
LEFT JOIN 
 (select region_code  province_code,province from csx_ods.source_crm_w_a_sys_province)b on a.province_name=b.province
;
--
-- select province_code,province_name, count(DISTINCT CASE
   ...
   GROUP BY a.province_code,province_name,a.goods_code,
    ...
    SELECT province_code,province_name,goods_code,
          ...
          0 AS sale_sku,
           ...
           sum( case when profit <0  then profit end ) AS negative_value,
          0 as day_sale,
          0 as day_profit,
          0 AS sale,
          0 AS profit,
...
-- select * from  temp.factory_stock_03 where province_name like '重庆%';
 -- 计算工厂、车间销售额
...
as 
 SELECT province_code,province_name,
       bd_id,bd_name,
       sum(sale_sku) AS sale_sku,
       count(DISTINCT   case when coalesce(sale,0) !=0 or period_amt!=0 then  goods_code  end  ) all_sku,
       sum(day_sale)day_sale,
      sum(day_profit) day_profit,
       sum(sale) AS sale,
       sum(profit) AS profit,
...
nvl(province_code,'')    	province_code ,
	...
	coalesce(negative_value,0) as negative_value,
...
a 
left join
(select distinct workshop_code,workshop_name from csx_dw.factory_bom where sdt=regexp_replace(date_sub(current_date(),1),'-','') and workshop_code!='H09') as b 
...
SELECT
	...
	coalesce(day_turnover  ,0)day_turnover,
	coalesce(negative_sku  ,0)negative_sku,
	coalesce(negative_value,0) negative_value,
	coalesce(receive_amt,0)receive_amt,
	0 shop_dept_cust,
     0   big_dept_cust,
       0 sale_cust_ratio,
...
union all 
	SELECT nvl(province_code,'')     province_code,
 province_name,'00' workshop_code,'全渠道' as workshop_name,
0 	sale_sku      ,
0 	all_sku       ,
0 day_sale,
0 day_profit,
...
-- 工厂产值
DROP TABLE IF EXISTS temp.fact_amt;

CREATE
TEMPORARY TABLE IF NOT EXISTS temp.fact_amt AS
SELECT a.province_code,
       a.workshop_code,
       a.workshop_name,
       sum(reality_receive_qty) AS reality_receive_qty,
       sum(fact_qty) AS fact_qty,
       sum(a.fact_values) as fact_values,
       sum(a.p_transfer_values)AS p_transfer_values,
       sum(a.transfer_quantity)AS transfer_qty
FROM csx_dw.factory_order a
WHERE sdt>= ${hiveconf:sdate}
  AND sdt<= ${hiveconf:edate}
...
-- 计算非工厂销售
-- select * from temp.factory_no_fact_01;
 drop table if exists temp.factory_no_01;
CREATE temporary table temp.factory_no_01
as 
     SELECT province_code,province_name,
      '非加工'workshop_name,
       sum(sale_sku) AS sale_sku,
       ...
       sum(receive_amt)as receive_amt
FROM
temp.factory_stock_03 where label ='0' 
GROUP BY province_code,province_name
;
...
group by  province_code,province_name) b on a.province_code=b.province_code and a.province_name=b.province_name
left join 
(select distinct workshop_code,workshop_name from csx_dw.factory_bom where sdt=regexp_replace(date_sub(current_date(),1),'-','') AND workshop_code!='H09') c on a.workshop_name=c.workshop_name
;

-- 非工厂销售
drop table if exists temp.fact_no_sale_all;
create temporary table if not exists temp.fact_no_sale_all as
select 
	province_code ,
	province_name ,
	workshop_code,
...
substr(${hiveconf:edate},1,6)months,
	...
	sum(sale_sku ) sale_sku ,
	sum(all_sku  )all_sku,	
	coalesce(sum(sale_sku )/sum(all_sku  ),0) as pin_rate,
    sum(day_sale)day_sale,
    sum(day_profit) day_profit,
    coalesce(sum(day_profit)/ sum(day_sale),0) day_profit_rate,
	sum(sale     )sale,	
	sum(profit   )profit,	
	coalesce(sum(profit   )/sum(sale),0) profit_rate,
	sum(mom_sale ) mom_sale,
	sum(mom_profit  )  mom_profit,
	coalesce(sum(mom_profit  ) /	sum(mom_sale ),0) as mom_profit_rate,
	sum(yoy_sale    )yoy_sale    ,
	sum(yoy_profit  )yoy_profit  ,
	coalesce(sum(yoy_profit  ) /	sum(yoy_sale ),0) as yoy_profit_rate,
    ...
    sum( big_cust)big_cust,
    sum(mon_big_dept_cust)mon_big_dept_cust,
    coalesce(sum(reality_receive_qty),0)reality_receive_qty,
    coalesce(sum(fact_qty),0)fact_qty,
    coalesce(sum(fact_values),0) as fact_values,
    coalesce(sum(reality_receive_qty)/sum(fact_qty),0) as fact_rate,
    sum(b.transfer_qty)transfer_qty,
   sum(p_transfer_values) transfer_amt,
    ${hiveconf:edate}
    from (
select '1' order_no,
nvl(province_code,'')    	province_code ,
...
(select a.province_code,
       ...
       a.workshop_name,
       reality_receive_qty,
       fact_qty, fact_values,
       p_transfer_values,
       transfer_qty from temp.fact_amt a) b on a.province_code=b.province_code and a.workshop_code=b.workshop_code and a.workshop_name=b.workshop_name 
group by a.province_code ,
	province_name ,
	a.workshop_code ,
	a.workshop_name,order_no
	order by a.province_code,order_no,case when workshop_name ='工厂' then 1 when workshop_name='物流' then 3 when workshop_name='全渠道' then 4 end desc;

-- select * from csx_dw.provinces_kanban where sdt='20200212';
-- -- 	show create table csx_dw.provinces_kanban;
	
	drop table `csx_dw.provinces_kanban`;
