 ...
 -- 找出符合条件的包裹单
    ...
    (
      ...
      WHERE sdt >='20230301' AND delivery_type_code = 1 -- 配送
    ) a LEFT JOIN
    (
      SELECT
        sale_order_code, 
        ...
        select 
            ...
            --  and order_code!=''
              ...
              group by shipped_order_code,
                entrucking_code,
                send_date
    ) c ON a.sale_order_code = c.sale_order_code AND a.delivery_date = c.delivery_date AND a.entrucking_code = c.entrucking_code
...
(
  SELECT
    ...
    t2.delivery_date, t2.entrucking_code, t2.package_code,
    t1.sale_unit, t2.send_quantity, -- 销售单位出库数量
    t2.basic_unit, t2.send_qty, -- 基础单位出库数量
...
(
  SELECT
    inventory_dc_code, inventory_dc_name, delivery_date, customer_code, order_code, package_code, goods_code, sale_unit, basic_unit,
