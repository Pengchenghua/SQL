...
case when  (emp_sub_name LIKE '永辉彩食鲜%'or emp_sub_name like '彩食鲜共享%' or emp_sub_name like '彩食鲜-富平%') then SUBSTRING_INDEX(SUBSTRING_INDEX(org_name,'_',2),'_',-1)
 when  emp_sub_name LIKE '%大区%' and   emp_sub_name regexp '[A-Z]' then substr(emp_sub_name,1,char_length (emp_sub_name )-2) when  emp_sub_name regexp '[\\(]'   then substring(emp_sub_name,1, char_length(emp_sub_name )-6)
...
case when (emp_sub_name LIKE '永辉彩食鲜%'or emp_sub_name like '彩食鲜共享%' or emp_sub_name like '彩食鲜-富平%') then SUBSTRING_INDEX(SUBSTRING_INDEX(org_name,'_',2),'_',-1)
   	when  SUBSTRING_INDEX(replace(replace(org_name,'永辉彩食鲜发展有限公司_大区_',''),'省区_',''),'_',3) regexp('部|人') then SUBSTRING_INDEX(replace(replace(org_name,'永辉彩食鲜发展有限公司_大区_',''),'省区_',''),'_',2) 
    ...
    org_name,
    emp_sub_name
	from (select DISTINCT ee.id,  employee_code ,employee_name ,cost_center ,emp_sub_name , mobile ,ee.email,pos_title_name,pos_title_id,
	orgunit_code,
          pos_position_name
from     csx_b2b_ucenter.erp_employee ee 
