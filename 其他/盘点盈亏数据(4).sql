--20191211数据显示部分旧系统业务过账但有财务过账-不符合财务逻辑，经核查发现部分业务人员将Z03Z04的业务操作成311移库-直接将正常仓的商品移库至盘点仓
...
as
select sdt,goodsid,x.shop_id,tax_code,cycle_unit_price,y.taxrate,
...
where sdt>='20190101' and sdt<'20191101' and shop_id like 'W%')x 
join (select distinct shop_id from csx_dw.shop_m where sdt='current' and sales_belong_flag in ('4_企业购','5_彩食鲜'))b on x.shop_id=b.shop_id
left join b2b.dim_ztax y on x.tax_code=y.rt_taxcode;
...
sum(quant_b)pd_qty
from csx_ods.mseg_ecc_dtl_fct 
where sdt>='20191101'and movetype in ('Z03','Z04') and stor_loc<>'B999'
group by sdt,plant,stor_loc,goodsid,movetype,case when movetype='Z03' then '盘亏'else '盘盈'end
union all --本质为Z03但业务操作时直接从正常仓移库至B999
select sdt,plant,stor_loc,goodsid,movetype move_type,'盘亏'move_name,
sum(quant_b)pd_qty
from csx_ods.mseg_ecc_dtl_fct 
where sdt>='20191101'and movetype in ('311') and stor_loc='B999'
group by sdt,plant,stor_loc,goodsid,movetype
...
--新系统盘点数据
--新系统盘点数据中包含link_wms_move_type='201A'期初入库，link_wms_move_type in('119A','120A')原料成本转换
...
goods_code,
goods_name,
move_type,case when substr(move_type,1,3)='110' then '盘亏' else '盘盈'end move_name,
...
department_name,
goods_code,
goods_name,move_type,case when substr(move_type,1,3)='110' then '盘亏' else '盘盈'end;
...
from 
(select substr(sdt,1,6)smonth,plant,goodsid,stor_loc,movetype,
sum(quant_b)pd_qty,
sum(rthfees)pd_value
from csx_ods.mseg_ecc_dtl_fct 
where sdt>='20191101'and movetype in ('711','712') and stor_loc='B999'
group by substr(sdt,1,6),plant,goodsid,stor_loc,movetype)a 
...
goods_name,
move_type,
case when substr(move_type,1,3)='115' then '盘点过账-盘盈'else '盘点过账-盘亏' end move_name,
sum(case when move_type like '%B' then -1*qty else qty end)qty,
sum(case when move_type like '%B' then -1*amt else amt end)amt 
from  csx_dw.wms_accounting_stock_operation_item_m
where sdt>='20191101' 
...
goods_code,
goods_name,
move_type,case when substr(move_type,1,3)='115' then '盘点过账-盘盈'else '盘点过账-盘亏' end;
...
from csx_ods.mseg_ecc_dtl_fct 
where sdt>='20191101'and movetype in ('Z45','Z46') 
