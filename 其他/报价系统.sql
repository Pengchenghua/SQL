...
*/
---所有订单的采购报价的计算逻辑
csx_dw.middle_office_prices csx_dw.purchase_prices --经OH183260000003核查，中台订单中订单金额是使用订单数量*商品单价
...
CREATE table b2b_tmp.temp_sale0715 as
select
  a.order_no,
  --订单号
  refund_no,
  --退货单号
  sdt edate,
  -- 这个特别注意、 是以分区时间取值的    个人觉得应该用出库配送时间作为发货时间（delivery_time）
  dc_code,
  --履约dc编码
  '' prefer_dc_code,
  --库存dc编码
  receive_mode,
  --'收货方式，HOME:送货上门，SITE:门店自提',    订单表中只有home的方式和空白方式
  order_mode,
  ...
  --商品编码
  self_product_name,
  --自建商品名称', 这个基本上数据基本上为空  20190910那天数据共2万9千多条、不为空的465条
  tax_rate,
  --税率
  estimate_arrive_time,
  --预计到达时间
  case
    when refund_no is null
    or refund_no = '' then purchase_qty
    else -1 * refund_qty
  end purchase_qty,
  -- 购买数量
  case
    when refund_no is null
    or refund_no = '' then send_qty
    else -1 * refund_qty
  end send_qty,
  --	发货数量
  case
    when refund_no is null
    or refund_no = '' then sign_qty
    else -1 * refund_qty
  end sign_qty,
  --签收数量
  origin_price,
  ...
  --正常进价
  case
    when refund_no <> '' then -1 * refund_value
    else origin_value
  end total_sale,
  --原总金额
  case
...
CREATE table b2b_tmp.temp_order as
select
  a.order_no,
  --订单号
  refund_no,
  --退货单号
  edate,
  -- 这个特别注意、 是以分区时间取值的    个人觉得应该用出库配送时间昨晚发货时间（delivery_time）
  dc_code,
  --履约dc编码
  prefer_dc_code,
  --库存dc编码
  ...
  --最近进价（含税）
  b.moving_average_price,
  --移动平均价
  case
    when order_mode = 3 then cut_through_price --直通订单中台报价',
    when order_mode = 1 then through_price --直达订单中台报价',
    when order_mode = 0 then distribute_price --配送订单中台报价
    when order_mode = 2 then take_delivery_price
  end middle_office_prices -- '自提订单中台报价'   上面类型不同都是直接作为中台报价
from (
    select
      *
    from b2b_tmp.temp_sale0715
    where
      sdt >= ${ hiveconf :i_sdate } -- 日期大于等于 20190901 这个条件可以省略、上面建临时表的时候已经限制了日期条件
  ) a
join (
    select
      *,case
        when type = '福利单' then 'WELFARE'
        else 'NORMAL'
      end order_kind
    from csx_dw.goods_prices_m --采购报价表
    where
      sdt = regexp_replace(current_date, '-', '') --因为数据是全量、所以去头一天数据
  ) b on (
    a.dc_code = b.warehouse_code
    and a.goodsid = b.goods_code
    and a.order_kind = b.order_kind
  );
--使用商品编码、仓库编码、订单类型进行关联
  drop table b2b_tmp.temp_res011;
CREATE table b2b_tmp.temp_res011 as
select
  a.order_no,
  --订单号
  a.refund_no,
...
join --自join
  (
    ...
    and coalesce(a.refund_no, 0) = coalesce(b.refund_no, 0) --退货单号
    and a.goodsid = b.goodsid --商品编码
    and to_date(a.import_time) = to_date(b.import_time) --a表得导入时间 等于 B表得最大导入时间
  );
...
select
  ...
  '' guide_price,
  --指导价格
  a.origin_cost_price purchase_price,
  --采购报价	 在采购报价表中没有为0的数据
  '' last_put_supplier,
  --最近入库供应商',
  '' last_purchase_price,
  --最近进价（含税）
  a.origin_cost_price moving_average_price,
  --移动平均价
  a.origin_cost_price middle_office_prices,
  --中台报价
  sdt
from (
    select
    ...
    from b2b_tmp.temp_order
    where
      edate < regexp_replace(to_date(price_begin_time), '-', '') --日期小于采购报价时间 、业务取上一次报价时间、最近进价的字段作用就体现出来了、查看采购报价表、最近进价字段中有2千多条为0
  ) a
left join b2b_tmp.temp_res011 b on (
    a.order_no = b.order_no
    and coalesce(a.refund_no, 0) = coalesce(b.refund_no, 0)
    and a.goodsid = b.goodsid
  )
where
  b.purchase_price is null;
  /*
         --全部订单中该部分数据量太大，暂时不取
...
CREATE temporary table b2b_tmp.temp_sapsale as
select
  shop_id,
  goods_code goodsid,
  sdt,
...
select
  ...
  b.goodsname,
  case
    when self_product_name is not null
    and self_product_name <> '' then '是'
    else '否'
  end self_product,
  a.tax_rate,
  a.origin_price,
...
left join (
  ...
  ) e on lpad(a.cust_id, 10, '0') = lpad(e.customer_no, 10, '0')
left join (
    select
      sdt,
      shop_id,
      goodsid,
      sale / qty price,(sale - profit) / qty cost_avg,
      profit / qty profit_avg
    from b2b_tmp.temp_sapsale
    where
      qty > 0
  ) c on (
    a.dc_code = c.shop_id
...
where
  a.send_qty <> 0
  and receive_mode is not null
  and edate <= ${ hiveconf :i_date };
select
  e.sales_province,
  dept_id,
  dept_name,
...
CREATE temporary table b2b_tmp.temp_pur as
select
  a.shop_id_in,
  a.goodsid,
  round(
    sum(pur_order_total_value) / sum(pur_order_qty),
    2
  ) last_storage_price
from (
    select
    ...
    where
      sdt >= concat(
        substr(
          regexp_replace(
            add_months(date_sub(current_date, 1), -3),
            '-',
            ''
          ),
          1,
...
select
  case
    when e.last_storage_price is not null then '近3个月最后一次入库价'
    else '商品资料表价'
  end price_type,
  a.dc_code,
  a.goodsid,
  ...
  d.shop_name,
  case
    when b.dept_id like 'H%'
    and e.last_storage_price is not null then e.last_storage_price
    when new_pur_price > 0 then new_pur_price
    when new_pur_price = 0
    and original_pur_price > 0 then original_pur_price
    else price_no_lvl
  end pur_price,
  case
    when new_sale_price > 0 then new_sale_price
    when new_sale_price = 0
    and original_sale_price > 0 then original_sale_price
    else price_no_lvl
  end sale_price
from (
    select
...
left join b2b_tmp.temp_pur e on (
    a.goodsid = e.goodsid
    and c.shop_id = e.shop_id_in
  );
id string comment '主键ID',
  ...
  sdt string comment '时间分区' --中台报价配置，不同的配送类型加上不一样的费率
  --订单模式：0-配送,1-直送，2-自提，3-直通
  order_kind订单类型 ： NORMAL - 普通单 ， WELFARE - 福利单 order_mode订单模式 ： 0 - 配送,
  1 - 直送 ， 2 - 自提 ， 3 - 直通