...
select
    ...
    channel,
    attribute_code,
    count(distinct case when sales_value>0 and sdt between  regexp_replace(${hiveconf:sdate},'-','') 
        and  regexp_replace(${hiveconf:edate},'-','') then a.customer_no end  )as all_sale_cust,
    count(distinct case when sales_value>0 and sdt between  regexp_replace(${hiveconf:l_sdate},'-','') 
        and  regexp_replace(${hiveconf:l_edate},'-','') then a.customer_no end  )as last_all_sale_cust
...
insert overwrite table csx_tmp.ads_sale_r_d_zone_classify_sale_fr partition(months,sdt)
...
-- 插入汇总数据
insert into table csx_tmp.ads_sale_r_d_zone_classify_sale_fr partition(months,sdt)
select
    level_id,
    substr(regexp_replace(${hiveconf:edate},'-',''),1,6) as sales_month,
    a.zone_id,
