...
drop table csx_dw.temp_account_in_01;
 ...
 as
 select a.hkont,a.comp_code,kunnr,prctr    ,sum(a.income)amount,sum(case when sale<=0 then sale+income end )as write_amt,
 ...
 -- 回款金额 income计算
drop table csx_dw.temp_account_in_03;
 CREATE temporary table csx_dw.temp_account_in_03
 as
 select a.hkont,a.comp_code,kunnr,prctr    ,sum(a.income)amount
 from csx_dw.temp_ecc_ytbcustomer  a 
 where
 1=1
 group by a.hkont,a.comp_code,prctr    ,kunnr;
...
-- 应收金额金额为正剔除冲销--弃
drop table if exists csx_dw.temp_account_out_02
;
CREATE temporary table if NOT EXISTS csx_dw.temp_account_out_02 as
select
	a.*,
	row_number() OVER(PARTITION BY hkont,comp_code,kunnr,prctr ORDER BY
					  budat asc)rno,
...
-- select * from  csx_dw.temp_account_in_02 where kunnr='102853';
-- 回款+冲销金额 --弃
drop table csx_dw.temp_account_in_02;
 CREATE temporary table csx_dw.temp_account_in_02
 as
 select a.hkont,a.comp_code,kunnr,prctr    ,sum(a.dmbtr)amount
 from csx_dw.temp_ecc_ytbcustomer  a 
 where
 dmbtr<0
 group by a.hkont,a.comp_code,prctr    ,kunnr;
 
--  select * from  csx_dw.temp_account_left_02 where kunnr='102853';
 -- 计算逾期数据 
