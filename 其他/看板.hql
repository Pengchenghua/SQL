    ...
    SELECT
      ...
      sum(
        case
          ...
          and sdt >= regexp_replace(${hiveconf:sdate }, '-', '') then coalesce(sales_value, 0)
        end
      ) AS sales_value,
      sum(
        case
          when sdt <= regexp_replace(add_months(${hiveconf:edate }, -1), '-', '')
          and sdt >= regexp_replace(add_months(${hiveconf:sdate }, -1), '-', '') then coalesce(sales_value, 0)
        end
      ) AS ring_sales_value,
      sum(
        case
          when sdt <= regexp_replace(${hiveconf:edate }, '-', '')
          and sdt >= regexp_replace(${hiveconf:sdate }, '-', '') then coalesce(sales_cost, 0)
        end
      ) as sales_cost,
      sum(
        case
          when sdt <= regexp_replace(${hiveconf:edate }, '-', '')
          and sdt >= regexp_replace(${hiveconf:sdate }, '-', '') then coalesce(profit, 0)
        end
      ) AS profit
    FROM csx_dw.sale_goods_m1 a
    WHERE
      sdt >= regexp_replace(add_months(${hiveconf:sdate }, -1), '-', '')
...
GROUP BY
  ...
  province_name,
  category_code;
--select * from temp.stock_04;
  --
  -- select a.*,b.* from temp.stock_03 a
  -- join
  -- (select * from csx_dw.goods_m where sdt='20191221'   )b
  -- on a.goods_code=b.goods_id
  -- and a.province_code='500000';
  --数
...
select
  ...
  coalesce(
    (sales_value - ring_sales_value) / ring_sales_value,
    0
  ) ring_sales_rate,
  profit / 10000 profit,
  profit_rate,
