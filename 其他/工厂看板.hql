...
SET sdate='2019-12-01';


SET edate='2019-12-23';
...
LEFT JOIN
(select receive_location_code,goods_code,sum(receive_qty*price) as receive_amt from csx_dw.wms_entry_order_m where sdt=regexp_replace(${hiveconf:edate},'-','')
...
DROP TABLE IF EXISTS temp.factory_stock_021;
...
from (
	...
	goods_code,
	category_code,
		CASE when sdt >= '20200101' and sdt <= '20200107' then sales_value end  sale,
		CASE when sdt >= '20200101' and sdt <= '20200107' then profit end profit,
		CASE when sdt >= '20191201' and sdt <= '20191207' then sales_value end  mom_sale,
		CASE when sdt >= '20191201' and sdt <= '20191207' then profit end  mom_profit,
		CASE when sdt >= '20190101' and sdt <= '20190107' then sales_value end  yoy_sale,
		CASE when sdt >= '20190101' and sdt <= '20190107' then profit end  yoy_profit
		from
...
--select * from  temp.factory_stock_03;
...
select a.province_code,a.workshop_code,a.workshop_name,user_qty,fact_qty,product_rate,(1/product_rate-1)as precision_rate,b.fact_amt,plan_qty from 
(SELECT province_code,workshop_code,workshop_name,
        sum(user_qty)user_qty,
       sum(fact_qty)fact_qty,
           sum(plan_user)/sum(fact_qty) AS product_rate,
          sum(plan_user)plan_qty
           --1/(sum(plan_user)/sum(fact_qty) AS precision_rate
FROM csx_dw.factory_out_rate
WHERE sdt>=regexp_replace(${hiveconf:sdate},'-','') and sdt<=regexp_replace(${hiveconf:edate},'-','')
GROUP BY province_code,workshop_code,location_code,workshop_name
)a
left join
(select province_code ,workshop_code, sum(p_cost_subtotal) fact_amt from csx_dw.factory_order_cost_materials
    where   sdt<=regexp_replace(${hiveconf:edate},'-','') and  sdt>=regexp_replace(${hiveconf:sdate},'-','')  
    group by  province_code ,workshop_code ) b on a. province_code =b.province_code
...
;
 ...
 a.province_code,
       ...
       ring_sales_value/10000 ring_sales_value,
       coalesce((sales_value-ring_sales_value)/ring_sales_value,0) as ring_sale_rate,
...
SELECT '1' no,
       ...
       a.sales_value,
       ring_sales_value,
       coalesce((a.sales_value-ring_sales_value)/ring_sales_value,0) as ring_sales_rate,
       a.profit,
...
LEFT JOIN
  (SELECT province_code,
   ...
   WHERE des_specific_product_status ='0' GROUP BY province_code,
          workshop_code)c ON a.province_code=c.province_code
AND a.workshop_code=c.workshop_code 
left join 
temp.temp_fact_01 d on a.province_code=d.province_code and a.workshop_code=d.workshop_code
union all 
SELECT '2' no,a.province_code,province_name,'' workshop_code,
       ''workshop_name,
        day_sale,
        day_profit,
      ...
      negative_sku,
       ...
       fact_amt,
       product_rate,
      (1/product_rate-1)as precision_rate
       
FROM  
(SELECT a.province_code,province_name,
       sum(sale_sku) AS sale_sku,
       --count(DISTINCT goods_code) all_sku,
       sum(day_sale) AS day_sale,
       sum(day_profit) AS day_profit,
       ...
       sum(negative_sku) as negative_sku,
       sum(receive_amt)as receive_amt
FROM  temp.factory_stock_03 a where a.workshop_code is not null
GROUP BY a.province_code,province_name
)a
LEFT JOIN
  (SELECT province_code,
          count(DISTINCT goods_code)all_sku
 ...
 LEFT JOIN 
(select province_code,
  count( distinct case when channel_name ='商超' then customer_no end) as shop_cust  ,
  count( distinct case when channel_name ='大客户' then customer_no end) as big_cust  from temp.factory_stock_02
group by  province_code) c on a.province_code=c.province_code
left join 
(SELECT province_code,
       sum(user_qty)user_qty,
       sum(fact_qty)fact_qty,
       sum(fact_amt)fact_amt,
       sum(plan_qty)/sum(fact_qty) AS product_rate,
       sum(plan_qty)plan_qty
     --  (1/(sum(fact_qty)/sum(plan_qty))-1) AS precision_rate
FROM temp.temp_fact_01
GROUP BY province_code) d on a.province_code=d.province_code  
) a order by province_code,no,workshop_code
;
